...
class Z3mMem(object):
    ...
    def get_mem_array(self, size):
        ...
        Return a z3 Array: BitVecSort(size) -> BitVecSort(9).
        """
        try:
            mem = self.mems[size]
        except KeyError:
...
class TranslatorZ3m(Translator):
    ...
    def __init__(self, endianness="<", loc_db=None, **kwargs):
        ...
        super(TranslatorZ3m, self).__init__(**kwargs)
        self._mem = Z3mMem(endianness)
        self.loc_db = loc_db
        ...
        if len(args) > 1:
            for arg in args[1:]:
                if expr.op in self.trivial_ops:
                    ...
                    print "jqelhjahzkdljhzajkdlamdlaji"
                    import IPython
                    IPython.embed()
                    res = args[-1]
                else:
...
# Register the class
Translator.register(TranslatorZ3m)
